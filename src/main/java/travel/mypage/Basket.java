...
//		}
		...
		if(n == 1) {
			...
			pagebar += String.format(" <li><a href='/SIST2_Travel/mypage/basket.do?page=%d' aria-label='Previous'> <span aria-hidden='true'>&laquo;</span></a></li> ", n-1);			
		}
...
//		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if (n == nowPage) {
				...
				pagebar += String.format(" <li><a href='/SIST2_Travel/mypage/basket.do?page=%d'>%d</a></li> ", n, n);
			}
...
//		
		...
		if(n > totalPage) {
			...
			pagebar += String.format(" <li><a href='/SIST2_Travel/mypage/basket.do?page=%d' aria-label='Next'> <span aria-hidden='true'>&raquo;</span></a></li> ", n);			
		}
		...
		//날짜 자르는 업무
		//1. SQL > 선택 X(O)
		//2. DAO > 선택 X or DAO에서도 데이터 가공은 안한다. 되도록 가공은 서블릿에게 맡긴다.
		...
		//2.
		ArrayList<BasketDTO> list = dao.list(map);
