...
@WebServlet("/community/clubboard/list.do")
public class List extends HttpServlet {
	...
	private void doPostGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (n == 1) {
			...
			pagebar += String.format(
					" <li><a href='/SIST2_Travel/community/clubboard/list.do?page=%d' aria-label='Previous'> <span aria-hidden='true'>&laquo;</span></a></li>",
					n - 1);
		}
			...
			while (!(loop > blockSize || n > totalPage)) {
				if (n == nowPage) {
					...
					pagebar += String.format(" <li><a href='/SIST2_Travel/community/clubboard/list.do?page=%d'>%d</a></li> ", n, n);
				}
				loop++;
				n++;
			}
			...
			if (n > totalPage) {
				...
				pagebar += String.format(
						" <li><a href='/SIST2_Travel/community/clubboard/list.do?page=%d' aria-label='Next'> <span aria-hidden='true'>&raquo;</span></a></li> ",
						n);
			}
			...
			ArrayList<ClubBoardDTO> list = dao.list(map);
			...
			RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/community/clubboard/list.jsp");
